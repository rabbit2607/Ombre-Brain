...
from utils import clean_llm_json, count_tokens_approx, positive_float, get_ai_name
...
def _perspective_rule(human: str, ai_name: str = "") -> str:
    _ai = ai_name or get_ai_name()
    # 第三人称模式：AI_NAME 已配置为真实名字（不是默认 "AI"）
    if _ai and _ai != "AI":
        return (
            "\n\n【归因铁律——最高优先级，违反即视为压缩失败】\n"
            f"以下内容以第三人称记录，涉及两方：「{human}」（用户）和「{_ai}」（AI）。\n"
            f"- AI 那一方一律称呼「{_ai}」，禁止用「我」「助手」「TA」「AI」代替。\n"
            f"- 用户那一方一律称呼「{human}」，禁止用「用户」「TA」代替。\n"
            "- 谁做的动作、谁的感受，就归到谁名下，不得混同或对调。\n"
            f"  ✗ 错：我回应了{human}的问题 / 用户表达了担忧\n"
            f"  ✓ 对：{_ai}回应了{human}的问题 / {human}表达了担忧"
        )
    # 第一人称模式：原始行为，AI 用「我」
    return (
...
【对话归因规则——输入为对话记录时必须遵守，最高优先级】
若输入每行格式为"[YYYY-MM-DD HH:MM] 说话人：内容"，则：
(1) 所有总结使用第三人称，主语直接写说话人名字（如"林野"、"小克"），
    严禁使用"我"、"用户"、"AI"作为主语；
(2) 严格按行首说话人前缀归因，绝对不能把一方说的话或属性记到另一方名下；
(3) content 字段必须以事件日期开头，格式严格为"YYYY-MM-DD：……"，
    日期取自消息行首的[YYYY-MM-DD HH:MM]时间戳，禁止省略或使用当前时间。
    例："2026-06-28：林野在厨房抽烟，感到难过……"
...
【归因规则】文本中"林野"是用户，"小克"是AI，suggested_name 中禁止用"我"或"用户"作主语。
